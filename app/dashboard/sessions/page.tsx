    ...
    <div className="max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-slate-600 mt-2">Track and manage your work sessions</p>
        </div>
        <Link
      ...
      {inProgressSessions.length > 0 && (
        <div className="mb-8">
          <h2 className="text-xl font-bold text-slate-900 mb-4">In Progress</h2>
          <div className="grid grid-cols-1 gap-4">
            {inProgressSessions.map((session: Session) => (
              <Link
                key={session.id}
                href={`/dashboard/sessions/${session.id}`}
                className="bg-white rounded-xl shadow-sm border-2 border-blue-500 p-6 hover:shadow-md transition-all"
              >
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-slate-600">📁 {session.projects?.name}</p>
      ...
      {!sessions || sessions.length === 0 ? (
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-12 text-center">
          ...
          <p className="text-slate-600 mb-6">Start your first work session to track proof of work</p>
          <Link
            href="/dashboard/sessions/new"
        ...
        <div>
          <h2 className="text-xl font-bold text-slate-900 mb-4">All Sessions</h2>
          <div className="grid grid-cols-1 gap-4">
