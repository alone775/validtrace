    ...
    <div className="max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-slate-600 dark:text-slate-400 mt-2">Manage your client projects</p>
        </div>
        <Link
      ...
      {!projects || projects.length === 0 ? (
        <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-12 text-center">
          ...
          <p className="text-slate-600 dark:text-slate-400 mb-6">Create your first project to start tracking work sessions</p>
          <Link
            href="/dashboard/projects/new"
          ...
          {projects.map((project: {
            ...
            >
              <div className="flex items-start justify-between mb-4">
                ...
                <span className={`px-3 py-1 rounded-full text-xs font-medium ${project.status === 'active' ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300' :
              ...
              <p className="text-sm text-slate-600 dark:text-slate-400 mb-4">Client: {project.client_name}</p>
              {project.description && (
                <p className="text-sm text-slate-500 dark:text-slate-500 mb-4 line-clamp-2">{project.description}</p>
              )}
